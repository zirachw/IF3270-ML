...
cells.append(md(
    ...
    "Empat parameter mengontrol perilaku modular, yaitu `INPUT_COLS` untuk fitur input "
    "(ubah ke beberapa kolom untuk mode *multivariate*), `VAL_STRATEGY` untuk strategi validasi, "
    "`INFER_MODE` yang menentukan cara prediksi data uji, di mana `'multistep'` memprediksi "
    "semua langkah sekaligus dalam satu *forward pass* sedangkan `'autoregressive'` menggunakan "
    "prediksi sebelumnya sebagai input untuk langkah berikutnya, serta `REGIME_FILTER` yang "
