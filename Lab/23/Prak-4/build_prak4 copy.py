...
cells.append(md(
    ...
    "Tiga parameter mengontrol perilaku modular, yaitu `INPUT_COLS` untuk fitur input "
    "(ubah ke beberapa kolom untuk mode *multivariate*), `VAL_STRATEGY` untuk strategi validasi, "
    "dan `INFER_MODE` yang menentukan cara prediksi data uji, di mana `'multistep'` memprediksi "
    "semua langkah sekaligus dalam satu *forward pass* sedangkan `'autoregressive'` menggunakan "
    "prediksi sebelumnya sebagai input untuk langkah berikutnya. "
...
cells.append(md(
    ...
    "Skala yang sama kemudian diterapkan pada data validasi dan data uji."
))
cells.append(code(
    "scaler = MinMaxScaler(feature_range=(0, 1))\n"
    "train_values = train_clean[CFG.TARGET_COL].values.reshape(-1, 1)\n"
    "scaled_values = scaler.fit_transform(train_values).flatten()\n\n"
...
cells.append(code(
    ...
    "    d0 = train_scaled_df.index[vi[0]  + CFG.WINDOW_SIZE].date()\n"
    "    d1 = train_scaled_df.index[vi[-1] + CFG.WINDOW_SIZE].date()\n"
...
cells.append(code(
    ...
    "def run_cv(model_class, checkpoint_path, label):\n"
    "    fold_rmse  = []\n"
    "    last_hist  = []\n"
    ...
    "        ).flatten()\n"
    "        last_dates  = train_scaled_df.index[val_idx + CFG.WINDOW_SIZE]\n"
...
cells.append(code(
    ...
    "    run_cv(RNNModel, CFG.CHECKPOINT_RNN, 'RNN')\n"
...
cells.append(code(
    ...
    "    run_cv(LSTMModel, CFG.CHECKPOINT_LSTM, 'LSTM')\n"
